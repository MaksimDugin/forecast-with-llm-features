...
2. Перед началом задача получает:
   - `ID`;
   - приоритет `P0 / P1 / P2`;
   - статус;
   - владельца;
...
```text
P0.1 Замечания научного руководителя
  ↓
P0.2 Формальная постановка задачи
  ↓
P0.3 Аудит notebook 4aapl.ipynb
  ↓
...
### Definition of Done для блока METH
...
### Definition of Done для блока CODE audit

- [ ] Понятно, что делает каждая часть notebook.
- [ ] Найдены все места, связанные с target.
- [ ] Найдены все места, связанные с признаками.
- [ ] Найдены все leakage-риски.
- [ ] Составлен список минимальных исправлений.
...
### Definition of Done для блока DATA

- [ ] Один актив выбран.
- [ ] Данные можно скачать заново.
- [ ] Период и число наблюдений известны.
- [ ] Пропуски обработаны по явному правилу.
- [ ] Таблица признаков сохранена.
...
### Definition of Done для блока FEAT

- [ ] Есть prices-only группа.
- [ ] Есть LLM-feature группа.
- [ ] Есть хотя бы одна теоретическая текстовая группа.
- [ ] Для каждого признака известно, когда он доступен.
...
### Definition of Done для блока MODEL

- [ ] Есть majority baseline.
- [ ] Есть prices-only baseline.
- [ ] Есть LLM-feature модель.
- [ ] Есть combined модель.
- [ ] Все модели обучаются на одном split.
...
### Definition of Done для блока EVAL

- [ ] Есть единая таблица результатов.
- [ ] Есть baseline.
- [ ] Есть confusion matrix.
- [ ] Есть аккуратный вывод: улучшает / не улучшает.
...
- Definition of Done:
  - [ ] Все 10 замечаний разложены по категориям.
  - [ ] Для каждого замечания есть действие.
  - [ ] Для каждого действия есть task ID.
...
- Definition of Done:
  - [ ] Найдена ячейка с target.
  - [ ] Найдена ячейка с split.
  - [ ] Найдена ячейка со scaler.
  - [ ] Найдены LLM features.
  - [ ] Найдены leakage-риски.
...
| 2026-06-24 | INBOX-002 / CODE-001–006 | статический разбор `4aapl.ipynb`: target, split, features, leakage-риски | `NOTEBOOK_REVIEW_4AAPL.md` |
